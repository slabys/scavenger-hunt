...
import {
  ...
  IonCol,
  IonImg,
  IonActionSheet, IonItem, IonText, IonCard,
} from "@ionic/react";
...
const Overview: React.FC = () => {
  ...
  return (
    <IonPage>
      ...
      <IonContent>
        <IonTitle color={"success"} className={"ion-padding"}>Finished hunts!</IonTitle>
        {huntsList.filter((f) => f.finishTime).map((hunt, i) => <IonCard key={`finished Hunt-${i}`}>
          {hunt.startTime && <IonItem>
            <IonText><b>Start:</b> {getDateWithTime(hunt.startTime)}</IonText>
          </IonItem>
          }
          {!!hunt.startTime && <IonItem>
                ...
                if (typeof route?.answer === "string") {
                  ...
                  return route?.answer &&
                    <IonRow key={`fragment-${i}`}>
                      <IonText>{`${i}:`}</IonText>
                      <IonCol size="6">
                        <IonImg src={photos.find((f) => f.filepath === (route.answer as UserPhoto).filepath)?.webviewPath} />
                      </IonCol>
                    </IonRow>;
                }
              })}
          ...
          {hunt.finishTime && <IonItem>
            <IonText><b>Finish:</b> {getDateWithTime(hunt.finishTime)}</IonText>
          </IonItem>
          }
        </IonCard>)}
