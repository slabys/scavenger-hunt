		...
		<IonPage>
			...
			<IonContent fullscreen className="ion-padding-top">
				...
				{(searchList.length !== 0 ? searchList : huntsList).map((hunt, index) => {
					return (
						<IonCard key={`hunt-card-${hunt.id}-${index}`} className={"ion-padding-bottom"}>
							<IonCardHeader>
								<IonCardTitle>{hunt.name}</IonCardTitle>
								<IonCardSubtitle color={new Date(hunt.dueDate) < new Date() ? "danger" : "success"}>
									Due date: {getDateWithTime(hunt.dueDate)}
								</IonCardSubtitle>
							</IonCardHeader>
