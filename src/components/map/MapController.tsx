...
const MapController: React.FC<MapControllerProps> = ({ huntData, onUpdate }) => {
	...
	const [modal, closeModal] = useTypedIonModal<ModalProps>(
		Modal,
		// Modal component props
		{
			title: huntData.name,
			startModal:
				huntData.startTime === null
					? {
							handleStart: () => {
	...
	return (
		<>
			...
			{currLocation?.latlng && (
				<FeatureGroup>
					<Circle center={currLocation.latlng} radius={currLocation.accuracy} />
			...
			{!huntData.startTime && (
				...
				>
					...
					{currLocation?.latlng && (
						<Popup>
							You are {calcDistanceInM(map, currLocation.latlng, huntData.start.latlng).toFixed(2)}m far
							away
						</Popup>
					)}
				</FeatureGroup>
